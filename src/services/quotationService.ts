import { PrismaClient, Quotation, QuotationStatus, Contract, ContractStatus } from '@prisma/client';
...
class QuotationService {
  async createQuotation(tenantId: string, createdBy: string, data: CreateQuotationData): Promise<Quotation> {
    ...
    const taxes = data.taxes || (subtotal - discounts) * 0.1; // 10% default tax
  ...
  async getQuotations(tenantId: string, query: QuotationsQuery): Promise<{
  ...
  async getQuotationById(tenantId: string, quotationId: string): Promise<QuotationWithRelations> {
  ...
  async updateQuotation(tenantId: string, quotationId: string, data: UpdateQuotationData): Promise<Quotation> {
  ...
  async deleteQuotation(tenantId: string, quotationId: string): Promise<{ success: boolean }> {
    const quotation = await prisma.quotation.findFirst({
      where: { id: quotationId, tenantId },
      include: {
        contracts: true,
    ...
    if (quotation.contracts.length > 0) {
      throw new ValidationError('Cannot delete quotation with associated contracts');
    }
  ...
  async sendQuotation(tenantId: string, quotationId: string): Promise<Quotation> {
    const quotation = await prisma.quotation.findFirst({
      where: { id: quotationId, tenantId },
    });
  ...
  async markQuotationAsViewed(tenantId: string, quotationId: string): Promise<Quotation> {
    const quotation = await prisma.quotation.findFirst({
      where: { id: quotationId, tenantId },
    });
  ...
  async acceptQuotation(tenantId: string, quotationId: string, approvedBy: string): Promise<Quotation> {
    const quotation = await prisma.quotation.findFirst({
      where: { id: quotationId, tenantId },
    });
    ...
    if (!['SENT', 'VIEWED'].includes(quotation.status)) {
      throw new ValidationError('Only sent or viewed quotations can be accepted');
    }
  ...
  async rejectQuotation(tenantId: string, quotationId: string, reason?: string): Promise<Quotation> {
    const quotation = await prisma.quotation.findFirst({
      where: { id: quotationId, tenantId },
    });
    ...
    if (!['SENT', 'VIEWED'].includes(quotation.status)) {
      throw new ValidationError('Only sent or viewed quotations can be rejected');
    }
  ...
  async convertToContract(tenantId: string, quotationId: string, createdBy: string, contractData: ConvertToContractData): Promise<Contract> {
    const quotation = await prisma.quotation.findFirst({
      where: { id: quotationId, tenantId },
      include: {
        client: true,
    ...
    if (quotation.status !== 'ACCEPTED') {
      throw new ValidationError('Only accepted quotations can be converted to contracts');
    }

    if (!quotation.clientId) {
      throw new ValidationError('Quotation must be associated with a client to create a contract');
    }
  ...
  async getQuotationStats(tenantId: string): Promise<{
    total: number;
    byStatus: Array<{ status: QuotationStatus; count: number; percentage: number }>;
    totalValue: number;
  ...
  async duplicateQuotation(tenantId: string, quotationId: string, createdBy: string): Promise<Quotation> {
    ...
    // Create new quotation with same data but new number and draft status
    const newValidUntil = new Date();
    newValidUntil.setDate(newValidUntil.getDate() + 30); // 30 days from now
