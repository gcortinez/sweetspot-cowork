...
import { ServiceCategory, ServiceType, RequestStatus, RequestPriority } from '@prisma/client';
...
export class ServiceAnalyticsService {
  ...
  async trackServiceUsage(
    ...
    try {
      ...
      // For now, we'll enhance the existing ServiceRequest model with analytics data
      ...
      logger.debug('Service usage tracked', { tenantId, action: data.action, serviceId: data.serviceId });
    } catch (error) {
      logger.error('Failed to track service usage', { tenantId, data }, error as Error);
  ...
  async generateUsageReport(
    ...
    try {
      ...
      for (const [serviceId, data] of serviceMetrics) {
        ...
        const completedRequests = data.requests.filter((r: any) => r.status === RequestStatus.COMPLETED).length;
        const cancelledRequests = data.requests.filter((r: any) => r.status === RequestStatus.CANCELLED).length;
        ...
        const completionTimes = data.requests
          ...
          .map((r: any) => 
            (new Date(r.completedAt).getTime() - new Date(r.createdAt).getTime()) / (1000 * 60 * 60)
          );
        ...
        const totalRevenue = data.requests.reduce((sum: number, r: any) => sum + Number(r.totalAmount), 0);
        ...
        const firstHalfRequests = data.requests.filter((r: any) => new Date(r.createdAt) < midpoint).length;
        const secondHalfRequests = data.requests.filter((r: any) => new Date(r.createdAt) >= midpoint).length;
        
        let trendDirection: 'UP' | 'DOWN' | 'STABLE' = 'STABLE';
        if (secondHalfRequests > firstHalfRequests * 1.1) trendDirection = 'UP';
        else if (secondHalfRequests < firstHalfRequests * 0.9) trendDirection = 'DOWN';
      ...
      logger.error('Failed to generate usage report', { tenantId, startDate, endDate }, error as Error);
      throw error;
    }
  }
  ...
  async generateDashboardData(
    ...
    try {
      ...
      logger.error('Failed to generate dashboard data', { tenantId, startDate, endDate }, error as Error);
      throw error;
    }
  }
  ...
  async generateRecommendations(
    ...
    try {
      ...
      logger.error('Failed to generate recommendations', { tenantId, timeframe }, error as Error);
      throw error;
    }
  }
  ...
  async generateAdvancedAnalytics(
    tenantId: string,
    analysisType: 'DEMAND_FORECAST' | 'PRICE_OPTIMIZATION' | 'CUSTOMER_SEGMENTS' | 'MARKET_OPPORTUNITIES' | 'ALL' = 'ALL'
    ...
    try {
      ...
      logger.error('Failed to generate advanced analytics', { tenantId, analysisType }, error as Error);
      throw error;
    }
  }
