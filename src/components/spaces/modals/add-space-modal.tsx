...
import { Settings, DollarSign, Clock, Shield, Plus } from 'lucide-react'
import { createSpaceAction } from '@/lib/actions/space'
...
export function AddSpaceModal({ open, onOpenChange, onSuccess }: AddSpaceModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || 'Error al crear el espacio')
      }
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Agrega un nuevo espacio a tu cowork
          </DialogDescription>
        </DialogHeader>
