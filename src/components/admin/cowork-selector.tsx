  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="flex items-center space-x-3 px-4 py-2 bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-50 transition-colors min-w-[280px]"
      >
        <div className="flex items-center space-x-2 flex-1">
          {currentSelection?.icon && (
            <currentSelection.icon className={`h-4 w-4 ${
              currentSelection.isPlatform ? 'text-purple-600' : 'text-blue-600'
            }`} />
          )}
          <div className="text-left">
            <div className="font-medium text-gray-900 text-sm">
              {currentSelection?.name || 'Seleccionar vista'}
            </div>
