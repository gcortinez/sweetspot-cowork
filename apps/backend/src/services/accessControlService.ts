...
export class AccessControlService {
  private readonly JWT_SECRET =
    process.env.QR_JWT_SECRET || "fallback-secret-key";
  ...
  async generateQRCode(
    ...
    try {
      ...
      const validUntil = new Date(
        validFrom.getTime() + data.validFor * 60 * 60 * 1000
      );
  ...
  async scanQRCode(
    ...
    try {
      ...
      const accessValidation = await this.validateAccessRules(
        tenantId,
        qrCode,
        scanData.location
      );
      ...
      const userInfo = qrCode.user
        ...
        ? {
            ...
            name: `${qrCode.visitor.firstName} ${qrCode.visitor.lastName}`,
      ...
      throw new ValidationError(
        `Failed to scan QR code: ${(error as Error).message}`
      );
    }
  }
  ...
  async createAccessRule(tenantId: string, data: AccessRuleData) {
    try {
      ...
      throw new ValidationError(
        `Failed to create access rule: ${(error as Error).message}`
      );
    }
  }
  ...
  async getAccessRules(tenantId: string, zoneId?: string) {
    try {
      ...
      throw new ValidationError(
        `Failed to fetch access rules: ${(error as Error).message}`
      );
    }
  }
    ...
    try {
      ...
      return await prisma.occupancyTracking.update({
        ...
        data: {
          ...
          lastEntry:
            update.action === "ENTRY" ? update.timestamp : occupancy.lastEntry,
          lastExit:
            update.action === "EXIT" ? update.timestamp : occupancy.lastExit,
      ...
      throw new ValidationError(
        `Failed to update occupancy: ${(error as Error).message}`
      );
    }
  }
  ...
  async getCurrentOccupancy(
    ...
    try {
      ...
      throw new ValidationError(
        `Failed to get occupancy data: ${(error as Error).message}`
      );
    }
  }
    ...
    try {
      ...
      for (const rule of rules) {
        ...
        if (
          ...
          Object.keys(rule.timeRestrictions).length > 0
        ) {
          const now = new Date();
          ...
          if (restrictions.start && restrictions.end) {
            ...
            if (currentTime < startTime || currentTime > endTime) {
              violations.push(
                `Access outside allowed hours: ${restrictions.start}-${restrictions.end}`
              );
              allowed = false;
        ...
        if (rule.maxOccupancy && location) {
          ...
          const currentCount = occupancy.reduce(
            (sum, occ) => sum + occ.currentCount,
            0
          );
        ...
        if (qrCode.user && rule.membershipTypes && rule.planTypes) {
          const membershipTypes = Array.isArray(rule.membershipTypes)
            ? rule.membershipTypes
            : [];
          const planTypes = Array.isArray(rule.planTypes) ? rule.planTypes : [];

          if (membershipTypes.length > 0 || planTypes.length > 0) {
  ...
  async getUserQRCodes(tenantId: string, userId: string) {
    try {
      ...
      throw new ValidationError(
        `Failed to get user QR codes: ${(error as Error).message}`
      );
    }
  }
  ...
  async revokeQRCode(tenantId: string, qrCodeId: string, revokedBy: string) {
    try {
      ...
      throw new ValidationError(
        `Failed to revoke QR code: ${(error as Error).message}`
      );
    }
  }
  ...
  async getAccessLogs(
    ...
    try {
      ...
      throw new ValidationError(
        `Failed to get access logs: ${(error as Error).message}`
      );
    }
  }
}
