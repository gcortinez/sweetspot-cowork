...
export class VisitorService {
  ...
  async createVisitor(
    ...
    try {
      ...
      const validUntil =
        data.validUntil || new Date(now.getTime() + 8 * 60 * 60 * 1000); // 8 hours default
  ...
  async deleteVisitor(tenantId: string, visitorId: string): Promise<void> {
    try {
  ...
  async getVisitorByQRCode(
    ...
    try {
      ...
      logger.error(
        "Failed to get visitor by QR code",
        { tenantId, qrCode },
        error as Error
      );
      throw error;
  ...
  async getTodaysVisitors(
    ...
    try {
      ...
      logger.error(
        "Failed to get today's visitors",
        { tenantId, hostUserId },
        error as Error
      );
      throw error;
  ...
  async getActiveVisitors(tenantId: string): Promise<VisitorData[]> {
    try {
      const visitors = await prisma.visitor.findMany({
        where: {
          tenantId,
  ...
  async checkInVisitor(
    ...
    try {
      ...
      const visitor = await prisma.$transaction(async (tx) => {
        ...
        // Validate visit time
        if (
          now < existingVisitor.validFrom ||
          now > existingVisitor.validUntil
        ) {
        ...
        const updatedVisitor = await tx.visitor.update({
          ...
          data: {
            ...
            healthDeclaration:
              ...
              existingVisitor.healthDeclaration ||
              undefined,
    ...
    try {
      ...
      const visitor = await prisma.$transaction(async (tx) => {
        ...
        const actualDuration = existingVisitor.checkedInAt
          ? Math.floor(
              (now.getTime() - existingVisitor.checkedInAt.getTime()) /
                (1000 * 60)
            )
          : 0;
        ...
        await tx.visitorLog.create({
          data: {
            ...
            details: `Checked out at ${
              data.checkOutLocation || "main entrance"
            }`,
  ...
  async extendVisitorStay(
    ...
    try {
      ...
      logger.info("Visitor stay extended", {
        tenantId,
        visitorId,
        newValidUntil,
      });
  ...
  async getVisitorStatistics(
    ...
    try {
      ...
      const activeVisitors = visitors.filter(
        (v) => v.status === VisitorStatus.CHECKED_IN
      ).length;
      ...
      const byPurpose = Array.from(purposeCounts.entries()).map(
        ([purpose, count]) => ({
          purpose,
          count,
  ...
  private mapVisitorToData(visitor: any): VisitorData {
    return {
      ...
      accessZones: Array.isArray(visitor.accessZones)
        ? visitor.accessZones
        : [],
  ...
  async updateExpiredVisitors(tenantId: string): Promise<number> {
    try {
      ...
      logger.info("Updated expired visitors", {
        tenantId,
        count: result.count,
      });
      return result.count;
      ...
      logger.error(
        "Failed to update expired visitors",
        { tenantId },
        error as Error
      );
      throw error;
    ...
    try {
      ...
      await this.sendNotification(tenantId, {
        ...
        message: `${request.firstName} ${
          request.lastName
        } has requested to visit on ${request.expectedArrival.toLocaleDateString()}`,
    ...
    try {
      ...
      if (preRegistration.expectedDuration) {
        validUntil.setMinutes(
          validUntil.getMinutes() + preRegistration.expectedDuration
        );
      } else {
      ...
      const visitor = await this.createVisitor(tenantId, {
        ...
        accessZones: Array.isArray(preRegistration.accessZones)
          ? (preRegistration.accessZones as string[])
          : [],
      ...
      logger.error(
        "Failed to convert pre-registration",
        {
          tenantId,
          preRegistrationId,
        },
  ...
  async useAccessCode(
    ...
    try {
      const validation = await this.validateAccessCode(
        tenantId,
        code,
        location,
        ipAddress
  ...
  private async sendVisitorInvitation(
    ...
    try {
      ...
      logger.error(
        "Failed to send visitor invitation",
        {
          tenantId,
          preRegistrationId,
        },
  ...
  private checkTimeRestrictions(restrictions: any): boolean {
    ...
    if (restrictions.startTime && restrictions.endTime) {
      const [startHour, startMinute] = restrictions.startTime
        .split(":")
        .map(Number);
