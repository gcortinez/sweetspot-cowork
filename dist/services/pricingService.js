"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.pricingService = void 0;
const prisma_1 = require("../lib/prisma");
const errors_1 = require("../utils/errors");
class PricingService {
    async createPricingTier(tenantId, data) {
        const existingTier = await prisma_1.prisma.pricingTier.findFirst({
            where: { tenantId, level: data.level },
        });
        if (existingTier) {
            throw new errors_1.ValidationError(`Pricing tier with level ${data.level} already exists`);
        }
        const tier = await prisma_1.prisma.pricingTier.create({
            data: {
                ...data,
                tenantId,
                features: data.features,
                restrictions: data.restrictions,
                isActive: data.isActive ?? true,
            },
        });
        return tier;
    }
    async getPricingTiers(tenantId, includeInactive = false) {
        const where = { tenantId };
        if (!includeInactive) {
            where.isActive = true;
        }
        const tiers = await prisma_1.prisma.pricingTier.findMany({
            where,
            include: {
                pricingRules: {
                    where: { isActive: true },
                    orderBy: { createdAt: 'desc' },
                },
                planTiers: {
                    include: {
                        plan: true,
                    },
                },
            },
            orderBy: { level: 'asc' },
        });
        return tiers;
    }
    async getPricingTierById(tenantId, tierId) {
        const tier = await prisma_1.prisma.pricingTier.findFirst({
            where: { id: tierId, tenantId },
            include: {
                pricingRules: {
                    orderBy: { createdAt: 'desc' },
                },
                planTiers: {
                    include: {
                        plan: true,
                    },
                },
            },
        });
        if (!tier) {
            throw new errors_1.NotFoundError('Pricing tier not found');
        }
        return tier;
    }
    async updatePricingTier(tenantId, tierId, data) {
        const tier = await prisma_1.prisma.pricingTier.findFirst({
            where: { id: tierId, tenantId },
        });
        if (!tier) {
            throw new errors_1.NotFoundError('Pricing tier not found');
        }
        if (data.level && data.level !== tier.level) {
            const existingTier = await prisma_1.prisma.pricingTier.findFirst({
                where: { tenantId, level: data.level, id: { not: tierId } },
            });
            if (existingTier) {
                throw new errors_1.ValidationError(`Pricing tier with level ${data.level} already exists`);
            }
        }
        const updatedTier = await prisma_1.prisma.pricingTier.update({
            where: { id: tierId },
            data: {
                ...data,
                features: data.features,
                restrictions: data.restrictions,
            },
        });
        return updatedTier;
    }
    async deletePricingTier(tenantId, tierId) {
        const tier = await prisma_1.prisma.pricingTier.findFirst({
            where: { id: tierId, tenantId },
            include: {
                pricingRules: true,
                planTiers: true,
            },
        });
        if (!tier) {
            throw new errors_1.NotFoundError('Pricing tier not found');
        }
        if (tier.pricingRules.length > 0 || tier.planTiers.length > 0) {
            throw new errors_1.ValidationError('Cannot delete pricing tier with associated rules or plans');
        }
        await prisma_1.prisma.pricingTier.delete({
            where: { id: tierId },
        });
        return { success: true };
    }
    async createPricingRule(tenantId, data) {
        const tier = await prisma_1.prisma.pricingTier.findFirst({
            where: { id: data.tierId, tenantId },
        });
        if (!tier) {
            throw new errors_1.NotFoundError('Pricing tier not found');
        }
        const rule = await prisma_1.prisma.pricingRule.create({
            data: {
                ...data,
                tenantId,
                timeSlots: data.timeSlots || [],
                conditions: data.conditions,
                validFrom: data.validFrom ? new Date(data.validFrom) : undefined,
                validTo: data.validTo ? new Date(data.validTo) : undefined,
                isActive: data.isActive ?? true,
            },
            include: {
                tier: true,
            },
        });
        return rule;
    }
    async getPricingRules(tenantId, tierId) {
        const where = { tenantId };
        if (tierId) {
            where.tierId = tierId;
        }
        const rules = await prisma_1.prisma.pricingRule.findMany({
            where,
            include: {
                tier: true,
            },
            orderBy: { createdAt: 'desc' },
        });
        return rules;
    }
    async updatePricingRule(tenantId, ruleId, data) {
        const rule = await prisma_1.prisma.pricingRule.findFirst({
            where: { id: ruleId, tenantId },
        });
        if (!rule) {
            throw new errors_1.NotFoundError('Pricing rule not found');
        }
        const updatedRule = await prisma_1.prisma.pricingRule.update({
            where: { id: ruleId },
            data: {
                ...data,
                timeSlots: data.timeSlots || rule.timeSlots,
                conditions: data.conditions || rule.conditions,
                validFrom: data.validFrom ? new Date(data.validFrom) : undefined,
                validTo: data.validTo ? new Date(data.validTo) : undefined,
            },
            include: {
                tier: true,
            },
        });
        return updatedRule;
    }
    async deletePricingRule(tenantId, ruleId) {
        const rule = await prisma_1.prisma.pricingRule.findFirst({
            where: { id: ruleId, tenantId },
        });
        if (!rule) {
            throw new errors_1.NotFoundError('Pricing rule not found');
        }
        await prisma_1.prisma.pricingRule.delete({
            where: { id: ruleId },
        });
        return { success: true };
    }
    async calculatePrice(tenantId, request) {
        const plan = await prisma_1.prisma.plan.findFirst({
            where: { tenantId, type: request.planType },
            include: {
                planTiers: {
                    include: {
                        tier: true,
                    },
                },
            },
        });
        if (!plan) {
            throw new errors_1.NotFoundError('Plan not found');
        }
        let basePrice = plan.price.toNumber();
        let tier = null;
        if (request.pricingTierId) {
            tier = await prisma_1.prisma.pricingTier.findFirst({
                where: { id: request.pricingTierId, tenantId },
            });
            if (tier) {
                const planTier = plan.planTiers.find(pt => pt.tierId === tier.id);
                if (planTier) {
                    basePrice = planTier.basePrice.toNumber();
                }
            }
        }
        const quantity = request.quantity || 1;
        const duration = request.duration || 1;
        let adjustedPrice = basePrice;
        const adjustments = [];
        const discounts = [];
        if (tier) {
            const rules = await prisma_1.prisma.pricingRule.findMany({
                where: {
                    tenantId,
                    tierId: tier.id,
                    isActive: true,
                    OR: [
                        { validFrom: null },
                        { validFrom: { lte: new Date() } },
                    ],
                    AND: [
                        {
                            OR: [
                                { validTo: null },
                                { validTo: { gte: new Date() } },
                            ],
                        },
                    ],
                },
                orderBy: { createdAt: 'asc' },
            });
            for (const rule of rules) {
                if (this.ruleApplies(rule, request)) {
                    const adjustment = this.applyPricingRule(adjustedPrice, rule);
                    adjustedPrice = adjustment.newPrice;
                    adjustments.push({
                        type: rule.ruleType,
                        description: rule.name,
                        amount: adjustment.amount,
                        percentage: adjustment.percentage,
                    });
                }
            }
        }
        const cycleMultiplier = this.getBillingCycleMultiplier(request.billingCycle);
        adjustedPrice *= cycleMultiplier;
        if (cycleMultiplier !== 1) {
            adjustments.push({
                type: 'BILLING_CYCLE',
                description: `${request.billingCycle} billing discount`,
                amount: adjustedPrice - (basePrice * quantity * duration),
                percentage: ((cycleMultiplier - 1) * 100),
            });
        }
        const subtotalBeforeDiscounts = adjustedPrice * quantity * duration;
        if (request.discountCode) {
            const discount = await this.applyDiscountCode(tenantId, request.discountCode, subtotalBeforeDiscounts);
            if (discount) {
                discounts.push(discount);
            }
        }
        const totalDiscounts = discounts.reduce((sum, d) => sum + d.amount, 0);
        const subtotal = subtotalBeforeDiscounts - totalDiscounts;
        const taxes = subtotal * 0.1;
        const total = subtotal + taxes;
        return {
            basePrice,
            adjustments,
            discounts,
            subtotal,
            taxes,
            total,
            currency: 'USD',
            breakdown: {
                unitPrice: adjustedPrice,
                quantity,
                duration,
                adjustedPrice,
            },
        };
    }
    ruleApplies(rule, request) {
        if (rule.planType && rule.planType !== request.planType) {
            return false;
        }
        if (rule.spaceType && rule.spaceType !== request.spaceType) {
            return false;
        }
        if (rule.timeSlots && request.timeSlot) {
            const ruleSlots = rule.timeSlots;
            const requestSlot = request.timeSlot;
            const timeMatches = ruleSlots.some(slot => {
                const dayMatches = slot.days.some((day) => requestSlot.days.includes(day));
                const timeOverlaps = this.timeRangesOverlap(slot.start, slot.end, requestSlot.start, requestSlot.end);
                return dayMatches && timeOverlaps;
            });
            if (!timeMatches) {
                return false;
            }
        }
        const conditions = rule.conditions;
        for (const [key, value] of Object.entries(conditions)) {
            const requestValue = request.metadata?.[key];
            if (requestValue !== value) {
                return false;
            }
        }
        return true;
    }
    applyPricingRule(currentPrice, rule) {
        const modifier = rule.modifier.toNumber();
        switch (rule.modifierType) {
            case 'MULTIPLIER':
                const newPriceMultiplier = currentPrice * modifier;
                return {
                    newPrice: newPriceMultiplier,
                    amount: newPriceMultiplier - currentPrice,
                    percentage: (modifier - 1) * 100,
                };
            case 'FIXED_AMOUNT':
                const newPriceFixed = currentPrice + modifier;
                return {
                    newPrice: Math.max(0, newPriceFixed),
                    amount: modifier,
                };
            case 'REPLACEMENT':
                return {
                    newPrice: modifier,
                    amount: modifier - currentPrice,
                };
            default:
                return { newPrice: currentPrice, amount: 0 };
        }
    }
    getBillingCycleMultiplier(cycle) {
        switch (cycle) {
            case 'DAILY':
                return 1;
            case 'WEEKLY':
                return 6.5;
            case 'MONTHLY':
                return 28;
            case 'QUARTERLY':
                return 81;
            case 'YEARLY':
                return 300;
            default:
                return 1;
        }
    }
    async applyDiscountCode(tenantId, code, amount) {
        const discountCode = await prisma_1.prisma.discountCode.findFirst({
            where: {
                tenantId,
                code,
                isActive: true,
                validFrom: { lte: new Date() },
                validTo: { gte: new Date() },
            },
        });
        if (!discountCode) {
            return null;
        }
        if (discountCode.usageLimit && discountCode.usageCount >= discountCode.usageLimit) {
            return null;
        }
        if (discountCode.minAmount && amount < discountCode.minAmount.toNumber()) {
            return null;
        }
        let discountAmount = 0;
        const discountValue = discountCode.value.toNumber();
        switch (discountCode.discountType) {
            case 'PERCENTAGE':
                discountAmount = (amount * discountValue) / 100;
                break;
            case 'FIXED_AMOUNT':
                discountAmount = discountValue;
                break;
            case 'FREE_DAYS':
                discountAmount = (amount / 30) * discountValue;
                break;
        }
        if (discountCode.maxDiscount && discountAmount > discountCode.maxDiscount.toNumber()) {
            discountAmount = discountCode.maxDiscount.toNumber();
        }
        await prisma_1.prisma.discountCode.update({
            where: { id: discountCode.id },
            data: { usageCount: { increment: 1 } },
        });
        return {
            type: discountCode.discountType,
            description: discountCode.name,
            amount: discountAmount,
        };
    }
    timeRangesOverlap(start1, end1, start2, end2) {
        const parseTime = (time) => {
            const [hours, minutes] = time.split(':').map(Number);
            return hours * 60 + minutes;
        };
        const start1Minutes = parseTime(start1);
        const end1Minutes = parseTime(end1);
        const start2Minutes = parseTime(start2);
        const end2Minutes = parseTime(end2);
        return start1Minutes < end2Minutes && start2Minutes < end1Minutes;
    }
    async getPlansWithPricing(tenantId, pricingTierId) {
        const plans = await prisma_1.prisma.plan.findMany({
            where: { tenantId, isActive: true },
            include: {
                planTiers: {
                    include: {
                        tier: true,
                    },
                    where: pricingTierId ? { tierId: pricingTierId } : undefined,
                },
            },
        });
        return plans.map(plan => ({
            ...plan,
            tiers: plan.planTiers.map(pt => ({
                tier: pt.tier,
                basePrice: pt.basePrice.toNumber(),
                features: pt.features || [],
            })),
        }));
    }
    async bulkUpdatePrices(tenantId, updates) {
        const results = { updated: 0, errors: [] };
        for (const update of updates) {
            try {
                const planTier = await prisma_1.prisma.planTier.findFirst({
                    where: {
                        planId: update.planId,
                        tierId: update.tierId,
                        plan: { tenantId },
                    },
                });
                if (!planTier) {
                    results.errors.push({
                        planId: update.planId,
                        tierId: update.tierId,
                        error: 'Plan tier combination not found',
                    });
                    continue;
                }
                await prisma_1.prisma.planTier.update({
                    where: { id: planTier.id },
                    data: { basePrice: update.basePrice },
                });
                results.updated++;
            }
            catch (error) {
                results.errors.push({
                    planId: update.planId,
                    tierId: update.tierId,
                    error: error instanceof Error ? error.message : 'Unknown error',
                });
            }
        }
        return results;
    }
}
exports.pricingService = new PricingService();
//# sourceMappingURL=pricingService.js.map